...
std::vector<CoinPtr> CSVStorage::getAllCoins()const
{
    ...
    std::vector<CoinPtr> coins;
    size_t i = 0;
    for (auto& row : reader) {
        coins.push_back(std::make_shared<Coin>());
        fillCoin(coins[i++], row);
    }
    return coins;
}

size_t CSVStorage::writeCoins(std::vector<CoinPtr>& coins)
{
...
void fillCoin(CoinPtr coin, csv::CSVRow& row)
{
